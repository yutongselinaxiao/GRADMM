...
from sign_converter import *
from tasks import get_syn_task, get_task, process_task
...
class OurArguments(TrainingArguments):
    ...
    task_name: (
        ...
        # class name. We support the following task_name: SST2, RTE, CB, BoolQ,
    ...
    # Number of examples
    num_train: int = (
        0  # ICL mode: number of demonstrations; training mode: number of training
        # samples
    )
    num_dev: int = (
        None  # (only enabled with training) number of development samples
    )
    num_eval: int = None  # number of evaluation samples
    num_eval_to_keep: int = 0  # number of evaluation samples to be kept
    kept_eval_as_train: bool = False
    mix_train_val: bool = False
    num_train_sets: int = (
        None  # how many sets of training samples/demos to sample; if None and
        # train_set_seed is None, then we will sample one set for each evaluation
        # sample
    )
    train_set_seed: int = None  # designated seed to sample training samples/demos
    result_file: str = (
        None  # file name for saving performance; if None, then use the task name,
    ...
    no_auto_device: bool = (
        False  # do not load model by auto device; should turn this on when using
        # FSDP
    )
    ...
    # - sgd: SGD training
    only_train_option: bool = (
        True  # whether to only train the option part of the input
    )
    train_as_classification: bool = (
        False  # take the log likelihood of all options and train as
        # classification
    )
    ...
    # SeMO
    gradient_ratio: float = 0.3  # Backward / Total in SeMO
    select_method: (
        str
    ) = (  # Methods to select backward samples in SeMO.
        # Other options: "topk", "random", "backward_only", "sign"
        "random"
    )
    beta: float = (
        0.001  # beta in SeMO
        # e.g. final_grad = beta * mezo_grad + (1-beta) * backward_grad
    )
    debug_semo: bool = False  # debug mode for SeMO
    use_cpu: bool = False  # use CPU for SeMO

    # Also reporting the train accuracy and train loss
    ...
    num_beams: int = 5  # number of beams for generation
    top_k: int = None  # top-k for generation
    top_p: float = 0.95  # top-p for generation
    max_new_tokens: int = 15  # max number of new tokens to generate
    ...
    lp_early_stopping: bool = (
        False  # whether to do early stopping in linear probing
    )
    ...
    # Non-diff objective
    non_diff: bool = (
        False  # use non-differentiable objective
        # (only support F1 for SQuAD for now)
    )
...
class Framework:
    ...
    def forward(self, input_ids, option_len=None, generation=False):
        ...
        For generation tasks, return the generated text. This function is only for
        inference
        """
    ...
    def one_step_pred(self, train_samples, eval_sample, verbose=False):
        ...
        In ICL, use train_samples as demonstrations
        """
        ...
        # Encode (add prompt and tokenize) the sample;
        # if multiple-choice/classification, encode all candidates (options)
        ...
        # Calibration
        sfc_encoded_candidates = None
        sfc_option_lens = None
        ...
        if self.task.generation:
            ...
            if self.args.sfc or self.args.icl_sfc:
                # Calibrated probabilities (surface form competition;
                # https://arxiv.org/pdf/2104.08315.pdf)
                # log p(candidate | input) = log p_lm(candidate | input) -
                # log p_lm(candidate | sfc prompt)
                scores = [
    ...
    def evaluate(
        ...
        If one_train_set_per_eval_sample is True, then each eval sample has its own
        training (demonstration) set.
        """
        if one_train_set_per_eval_sample:
            logger.info(
                f"There are {len(eval_samples)} validation samples and one train set"
                " per eval sample"
            )
        else:
            logger.info(
                f"There are {len(train_samples)} training samples and"
                f" {len(eval_samples)} validation samples"
            )
        ...
        if metric_name == "accuracy":
            logger.info("Calculate accuracy per class")
            metrics["per_class_accuracy"] = calculate_metric(
                predictions, "per_class_accuracy"
            )
        return metrics
    ...
    def train(self, train_samples, eval_samples, test_samples):
        ...
        def _convert(samples):
            ...
            for sample in samples:
                ...
                if self.args.non_diff:
                    # For non-differentiable objective, there is no teacher forcing
                    # thus the current answer part is removed
                ...
                if self.args.train_as_classification:
                    ...
                    if self.args.non_diff:
                        ...
                        # we need to provide the gold answer to calculate F1/acc
                        data.append({
        ...
        if self.args.only_train_option and not self.args.non_diff:
            # If --only_train_option and not with a non-differentiable objective,
            # we wrap the forward function
        ...
        os.environ["WANDB_PROJECT"] = "gradmm"
        self.args.run_name = result_file_tag(self.args)

        if self.args.trainer == "sgd_in_place":
            fo_lr = self.args.learning_rate
            optimizer_dict = {
                p: torch.optim.SGD([p], foreach=False, lr=fo_lr)
        ...
        trainer = OurTrainer(
            model=self.model,
            optimizers=(
                (SGD(self.model.parameters(), lr=self.args.learning_rate), None)
                if self.args.trainer == "sgd"
                else (None, None)
            ),
            args=self.args,
        ...
        if self.args.only_train_option and not self.args.non_diff:
            if isinstance(self.model, FSDP):
                logger.info(
                    "This is an FSDP model now. Be careful when assigning back the"
...
def main():
    ...
    if args.local_rank <= 0:
        os.makedirs(os.path.join(addax_exp_root_path,
                    result_file_tag(args)), exist_ok=True)

        logging_path = os.path.join(
            addax_exp_root_path, result_file_tag(args), "log.log"
        )
        # logger.addHandler(logging.FileHandler(logging_path))
        stream = open(logging_path, "w")
    ...
    if args.train_set_seed is not None or args.num_train_sets is not None:
        ...
        for train_set_id, train_samples in enumerate(train_sets):
            ...
            if not args.no_eval:
                ...
                logger.info(metrics)
                if args.local_rank <= 0:
                    addax_exp_root_path = os.environ.get(
                        "ADDAX_EXP_ROOT_PATH", "result")
                    tmp_metrics_path = os.path.join(
                        addax_exp_root_path,
        ...
        if args.local_rank <= 0:
            ...
            tmp_metrics_path = os.path.join(
                addax_exp_root_path,
                result_file_tag(args),
                "onetrainpereval.json",
            )
            write_metrics_to_file(
                metrics,
                (tmp_metrics_path if args.result_file is None else args.result_file),
            )
            del tmp_metrics_path
